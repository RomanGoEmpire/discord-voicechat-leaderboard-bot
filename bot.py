...
class Bot(commands.Bot):
    ...
    async def on_message(self, message):
        ...
        if message.author.bot:
            return
        logging.info(f"{message.author.name} ({message.channel.name}): {message.content}")
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        if before.channel is None and after.channel is not None:
            ...
            if member.id in self.join_times:
                ...
                logging.info(f"Leave: {member.display_name} has left a voice channel after {convert_to_readable_time(duration)}")
